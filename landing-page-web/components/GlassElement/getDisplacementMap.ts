...
export const getDisplacementMap = ({
  ...
  depth,
  scale = 1,
}: Omit<DisplacementOptions, "chromaticAberration" | "strength"> & { scale?: number }) =>
  "data:image/svg+xml;utf8," +
  encodeURIComponent(`<svg height="${Math.max(1, Math.round(height * scale))}" width="${Math.max(1, Math.round(width * scale))}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision">
    <style>
    ...
    <g filter="blur(${Math.max(2, Math.round(scale))}px)">
      ...
      <rect
          ...
          rx="${radius}"
          ry="${radius}"
          filter="blur(${Math.max(depth, Math.round(scale))}px)"
      />
    </g>
</svg>`)
